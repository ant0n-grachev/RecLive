...
import {getChicagoTimestampMs} from "./chicagoTime";
...
const DATE_TOKEN_PATTERN =
    String.raw`(?:\d{4}-\d{2}-\d{2}|[a-z]{3,9}\s+\d{1,2}(?:,\s*\d{4})?|\d{1,2})`;
const DATE_RANGE_PATTERN = new RegExp(`^(${DATE_TOKEN_PATTERN})(?:\\s*-\\s*(${DATE_TOKEN_PATTERN}))?$`);
...
const NEXT_OPEN_SEARCH_DAYS = 365;
...
const timestampForDateMinute = (dateKey: string, minuteOfDay: number): number | null => {
    ...
    return getChicagoTimestampMs(`${dateKey}T${pad2(hour)}:${pad2(minute)}:00`);
};
...
const parseDateRangeLabel = (
    ...
    const endToken = match[2] ?? startToken;

    const start = parseTokenDate(startToken, fallbackYear);
    ...
    if (endKey < startKey) {
        ...
        if (shiftedEnd) {
            ...
            const shiftedStart = parseTokenDate(startToken, fallbackYear - 1);
            if (shiftedStart) {
...
export const getFacilityNextOpenTimestamp = (
    schedule: FacilityHoursFacilityPayload | null | undefined,
    now: Date = new Date()
): string | null => {
    if (!schedule || !Array.isArray(schedule.sections) || schedule.sections.length === 0) {
    ...
    for (let dayOffset = 0; dayOffset <= NEXT_OPEN_SEARCH_DAYS; dayOffset += 1) {
        const dayKey = shiftDateKeyByDays(todayKey, dayOffset);
        if (!dayKey) {
            continue;
        }

        const windows = getFacilityOpenWindowsForDate(schedule, dayKey);
        for (const window of windows) {
            if (dayOffset === 0 && window.startMinutes <= chicagoNow.minutes) {
                continue;
            }

            const timestampMs = timestampForDateMinute(dayKey, window.startMinutes);
            if (timestampMs === null) {
                continue;
            }
