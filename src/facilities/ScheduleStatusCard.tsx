import type {ReactNode} from "react";
import {Box, Chip, Stack, Typography} from "@mui/material";
import CheckCircleOutlineRoundedIcon from "@mui/icons-material/CheckCircleOutlineRounded";
import CloseRoundedIcon from "@mui/icons-material/CloseRounded";
import {CARD_SHELL_SX} from "../shared/utils/styles";
import type {FacilityOpenStatus} from "../shared/utils/facilityScheduleStatus";

interface ScheduleStatusCardProps {
    status: FacilityOpenStatus;
    nextOpenLabel?: string | null;
    footerAction?: ReactNode;
}

type KnownScheduleState = Exclude<FacilityOpenStatus["state"], "unknown">;

const STATUS_TEXT: Record<KnownScheduleState, string> = {
    open: "Open now according to the official schedule",
    closed: "Closed now according to the official schedule",
};

const STATUS_COLOR: Record<KnownScheduleState, "success" | "error"> = {
    open: "success",
    closed: "error",
};

const statusIcon = (state: KnownScheduleState) => {
    if (state === "open") {
        return <CheckCircleOutlineRoundedIcon fontSize="small"/>;
    }
    return <CloseRoundedIcon fontSize="small"/>;
};

export default function ScheduleStatusCard({
    status,
    nextOpenLabel = null,
    footerAction = null,
}: ScheduleStatusCardProps) {
    if (status.state === "unknown") {
        return null;
    }

    const detailText = status.matchedRule
        ? `${status.matchedRule.label}: ${status.matchedRule.hours}`
        : null;
    const nextOpenText = status.state === "closed" && nextOpenLabel
        ? `Opens next: ${nextOpenLabel}`
        : null;

    return (
        <Box
            sx={{
                ...CARD_SHELL_SX,
                p: {xs: 1.6, sm: 1.8},
            }}
        >
            <Stack spacing={1}>
                <Stack
                    direction="row"
                    alignItems="center"
                    justifyContent="space-between"
                    gap={1}
                    sx={{flexWrap: "wrap"}}
                >
                    <Typography variant="subtitle2" color="text.secondary">
                        Schedule Status
                    </Typography>
                    <Chip
                        size="small"
                        color={STATUS_COLOR[status.state]}
                        icon={statusIcon(status.state)}
                        label={status.state.toUpperCase()}
                        sx={{fontWeight: 700, flexShrink: 0}}
                    />
                </Stack>
                <Typography variant="body1" sx={{fontWeight: 700}}>
                    {STATUS_TEXT[status.state]}
                </Typography>
                {detailText && (
                    <Typography variant="body2" color="text.secondary">
                        {detailText}
                    </Typography>
                )}
                {nextOpenText && (
                    <Typography variant="body2" color="text.secondary" sx={{fontWeight: 700}}>
                        {nextOpenText}
                    </Typography>
                )}
                {footerAction && (
                    <Box sx={{display: "flex", justifyContent: "flex-end", pt: 0.25}}>
                        {footerAction}
                    </Box>
                )}
            </Stack>
        </Box>
    );
}
