...
const mergeActualHoursIntoDays = (
    ...
    if (!actualPayload?.date) {
        return days;
    }

    const byCategory = new Map<string, Map<string, ActualHourPayload>>();
    if (Array.isArray(actualPayload.categories)) {
    ...
    const totalHoursByStart = new Map<string, ActualHourPayload>();
    if (Array.isArray(actualPayload.totalHours)) {
        for (const hour of actualPayload.totalHours) {
            if (!hour?.hourStart) continue;
            totalHoursByStart.set(hour.hourStart, hour);
        }
    }
    ...
    return days.map((day) => {
        ...
        if (Array.isArray(day.totalHours) && totalHoursByStart.size > 0) {
            totalHours = day.totalHours.map((hour) => {
                const actualHour = totalHoursByStart.get(hour.hourStart);
                if (!actualHour) return hour;
                return {
                    ...hour,
                    actualCount: actualHour.actualCount,
