...
- **Polling:** independent clocks: live 90 seconds, forecast 15 minutes, schedule four hours. Hidden tabs pause requests while retaining due clocks. Only live refreshes immediately when visibility returns; overdue forecast/schedule refresh on the next probe (up to 30 seconds later). Offline mode disables polling; re-enabling recreates clocks, with only live refreshing immediately. Pull-to-refresh refreshes live only. Requests for each resource do not overlap. This reconciles the master wording with the accepted specification and Phase 6 policy; it does not require all resources to refresh on focus/reconnect.
